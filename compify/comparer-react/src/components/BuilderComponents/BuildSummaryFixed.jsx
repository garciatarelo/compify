...
function BuildSummary({ onClearBuild, onOpenComparison }) {
  ...
  const selectedComponents = Object.entries(currentBuild)
    ...
    .map(([categoryId, componentId]) => {
      ...
      // Obtener URL de la tienda seleccionada
    ...
    <div className="bg-white p-6 rounded-lg shadow-lg sticky top-24">
      ...
      <div id="build-summary" className="space-y-3 mb-6">
        {selectedComponents.length === 0 ? (
          ...
          selectedComponents.map(({ categoryId, categoryName, categoryIcon, selectedStore, selectedPrice, storeUrl, storeLogo }) => {
            return (
              <div key={categoryId} className="p-2 rounded hover:bg-gray-50 border border-gray-100">
                <div className="flex items-center gap-2 mb-1">
                  {getIconComponent(categoryIcon)}
                  <span className="text-sm text-gray-700 font-medium flex-1">{categoryName}</span>
